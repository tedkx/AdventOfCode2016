...
public class Room {
  ...
  const int LowercaseCharCodeA = 97;
  const int LowercaseCharCodeZ = 122;
  static int[] CharCodes = Enumerable.Range(LowercaseCharCodeA, LowercaseCharCodeZ - LowercaseCharCodeA + 1)
    .ToArray();
  
  string _input;
  
  public int SectorID { get; private set; }
  public bool IsNorthPoleStorage { get; private set; }
  ...
  public static char ShiftLetter(char letter, int offset) {
    if(letter == '-')
      return ' ';
    if(offset > CharCodes.Length)
      offset = offset % CharCodes.Length;
    var idx = Array.IndexOf(CharCodes, (int)letter) + offset;
    if(idx >= CharCodes.Length)
      idx -= CharCodes.Length;
    return (char)CharCodes[idx];
  }
  
  private void Process() {
    Match match = regex.Match(_input);
    
    string fakeName = match.Groups[1].Captures[0].Value;
    SectorID = int.Parse(match.Groups[2].Captures[0].Value);
    
    var realName = string.Join("", fakeName.Select(l => Room.ShiftLetter(l, SectorID)));
    if(realName.Contains("north") && realName.Contains("pole"))
      IsNorthPoleStorage = true;
  }
}
  
public class SecurityThroughObscurity {
  public Dictionary<char, int> Occurences;
  
  public SecurityThroughObscurity(string input) {
    foreach(var str in input.Split('\n')) {
      var room = new Room(str);
      if(room.IsNorthPoleStorage) {
        StorageRoomSectorID = room.SectorID;
        break;
      }
    }
  }
  
  public int StorageRoomSectorID { get; private set; }
...
hplazytkpo-prr-cpnptgtyr-379[prtya]";
    var sec = new SecurityThroughObscurity(input);
    Console.WriteLine(sec.StorageRoomSectorID);
