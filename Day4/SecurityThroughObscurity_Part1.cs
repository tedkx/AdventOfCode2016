...
public class Room {
  ...
  Dictionary<char, int> _occurences;
  string _input;
  ...
  public Room(string input) {
    _occurences = CharCodes.ToDictionary(k => (char)k, v => 0);  
    _input = input;
  ...
  private void Process() {
    ...
    string letters = match.Groups[1].Captures[0].Value.Replace(@"-", ""), 
      checksum = match.Groups[3].Captures[0].Value;
    ...
    // select top 5 occurences, tie breaking alphabetically
    var top5 = _occurences.OrderByDescending(pair => pair.Value)
...
public class SecurityThroughObscurity {
  ...
  public SecurityThroughObscurity(string input) {
    // create alphabet dictionary with initial occurence 0 for each letter
    var rooms = input.Split('\n').Select(str => new Room(str));
    RealRoomsSum = rooms.Where(r => r.IsReal).Select(r => r.SectorID).Sum();
  }
  
  public int RealRoomsSum { get; private set; }
...
hplazytkpo-prr-cpnptgtyr-379[prtya]";
    ...
    Console.WriteLine(sec.RealRoomsSum);
